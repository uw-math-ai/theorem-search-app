...
var PER_SOURCE_MULTIPLIER = 3;
function parsePaperFilter(raw) {
  const ids = [];
  const titles = [];
...
async function handleSearch({
  ...
  try {
    ...
    for (const source of sources) {
      const params = [source, vecStr, topK * PER_SOURCE_MULTIPLIER];
      const extra = [];
      const p = (val) => {
        params.push(val);
      ...
      if (hasCitationFilter) {
        ...
        const high = filters.citationMax ?? 2e6;
      ...
      if (filters.paperFilter?.trim()) {
        const { ids: ids2, titles } = parsePaperFilter(filters.paperFilter);
        const orClauses = [];
        if (ids2.length) orClauses.push(`paper_id LIKE ANY(${p(ids2.map((id) => id + "%"))})`);
      ...
      const sql = `
        ...
        FROM ann
      `;
      const { rows: rows2 } = await client.query(sql, params);
      all.push(...rows2);
    }
...
async function handleMetadata() {
  ...
  const [sourcesRes, authorsRes, tagsRes, countRes, yearRes, citationRes] = await Promise.all([
    pool2.query("SELECT sources FROM mv_sources"),
    pool2.query("SELECT source, authors FROM mv_authors_by_source"),
    pool2.query("SELECT source, tags FROM mv_tags_by_source"),
    pool2.query("SELECT cnt FROM mv_theorem_count"),
    pool2.query("SELECT MIN(year) AS year_min, MAX(year) AS year_max FROM theorem_search_qwen8b WHERE year IS NOT NULL"),
    pool2.query("SELECT MAX(citations) AS citation_max FROM theorem_search_qwen8b WHERE citations IS NOT NULL")
  ]);
  const authorsPerSource = {};
  for (const row of authorsRes.rows) authorsPerSource[row.source] = row.authors ?? [];
  const tagsPerSource = {};
  for (const row of tagsRes.rows) tagsPerSource[row.source] = row.tags ?? [];
  const data = {
    ...
    yearMax: Number(yearRes.rows[0]?.year_max ?? (/* @__PURE__ */ new Date()).getFullYear()),
    citationMax: Number(citationRes.rows[0]?.citation_max ?? 1e4)
  };
  metaCache = { data, expiresAt: now + 7 * 24 * 60 * 60 * 1e3 };
  return data;
}
async function handleFeedback(payload) {
  ...
  await getPool().query(
    ...
    [
      vote,
      String(query).trim().slice(0, 1e3),
      String(url).slice(0, 2e3),
      String(theorem_name).slice(0, 500),
      Array.isArray(authors) ? authors.join(", ").slice(0, 1e3) : null,
...
async function handleLogQuery(payload) {
  ...
  await getPool().query(
    ...
    [String(query).trim().slice(0, 1e3), sources, filtersJson]
  );
  return { ok: true };
}
function ok(data) {
...
var handler = async (event) => {
  ...
  try {
    ...
    switch (action) {
      case "search":
        return ok(await handleSearch(payload));
      case "metadata":
        return ok(await handleMetadata());
      case "feedback":
        return ok(await handleFeedback(payload));
      case "log-query":
        return ok(await handleLogQuery(payload));
      default:
